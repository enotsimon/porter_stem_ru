...
-export([rev_and_number/1]). % debug
-export([test/0]). % debug

test() ->
	{ok, Data} = file:read_file("src/test_data"),
	Words = [binary:split(D, <<",">>, [global]) || D <- binary:split(Data, <<"\n">>, [global])],
	TFun = fun(In, MustBe) ->
		Res = stem(In),
		Ok = case Res == unicode:characters_to_list(MustBe) of true -> "OK"; false -> "FAIL" end,
		lager:info("TEST ~p:  ~ts result: ~ts must_be: ~ts", [Ok, In, Res, MustBe]), timer:sleep(5)
	end,
...
lstem(Word) when is_list(Word) ->
	...
	%lager:info("WORD: ~ts   ~w", [Word, Word]),
	case get_rv_part(Word, []) of
		{Start, []} ->
			%lager:info("NO V R: ~ts   ~w", [Start, Start]),
			Start;
		{Start, RV} ->
			P = lists:reverse(step4(step3(step2(step1(lists:reverse(RV)))))),
			%lager:info("YES V R: {~ts, ~ts} {~w, ~w} result: ~ts", [Start, P, Start, P, Start++P]),
			Start++P
	end.
...
step1_2([1099, 1084, 1080 | Rest]) -> step1_3(Rest); % ыми -> ""
step1_2([1077, 1075, 1086 | Rest]) -> step1_3(Rest); % его -> ""
step1_2([1086, 1075, 1086 | Rest]) -> step1_3(Rest); % ого -> ""
step1_2([1077, 1084, 1091 | Rest]) -> step1_3(Rest); % ему -> ""
step1_2([1086, 1084, 1091 | Rest]) -> step1_3(Rest); % ому -> ""
...
step1_2([1081, 1090, 1077, V | Rest]) when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "йте" -> (а/я)
step1_2([1077, 1096, 1100, V | Rest]) when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ешь" -> (а/я)
step1_2([1083, 1072, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ла" -> (а/я)
step1_2([1085, 1072, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "на" -> (а/я)
step1_2([1083, 1080, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ли" -> (а/я)
step1_2([1077, 1084, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ем" -> (а/я)
step1_2([1083, 1086, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ло" -> (а/я)
step1_2([1085, 1086, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "но" -> (а/я)
step1_2([1077, 1090, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ет" -> (а/я)
step1_2([1102, 1090, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ют" -> (а/я)
step1_2([1085, 1099, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ны" -> (а/я)
step1_2([1090, 1100, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; % (а/я) "ть" -> (а/я)
...
step1_2([1077, 1081, 1090, 1077 | Rest]) -> Rest; % ейте -> ""
step1_2([1091, 1081, 1090, 1077 | Rest]) -> Rest; % уйте -> ""
step1_2([1080, 1083, 1072 | Rest]) -> Rest; % ила -> ""
step1_2([1099, 1083, 1072 | Rest]) -> Rest; % ыла -> ""
step1_2([1077, 1085, 1072 | Rest]) -> Rest; % ена -> ""
step1_2([1080, 1090, 1077 | Rest]) -> Rest; % ите -> ""
step1_2([1080, 1083, 1080 | Rest]) -> Rest; % или -> ""
step1_2([1099, 1083, 1080 | Rest]) -> Rest; % ыли -> ""
step1_2([1080, 1083, 1086 | Rest]) -> Rest; % ило -> ""
step1_2([1099, 1083, 1086 | Rest]) -> Rest; % ыло -> ""
step1_2([1077, 1085, 1086 | Rest]) -> Rest; % ено -> ""
step1_2([1091, 1077, 1090 | Rest]) -> Rest; % ует -> ""
step1_2([1091, 1102, 1090 | Rest]) -> Rest; % уют -> ""
step1_2([1077, 1085, 1099 | Rest]) -> Rest; % ены -> ""
step1_2([1080, 1090, 1100 | Rest]) -> Rest; % ить -> ""
step1_2([1099, 1090, 1100 | Rest]) -> Rest; % ыть -> ""
step1_2([1080, 1096, 1100 | Rest]) -> Rest; % ишь -> ""
step1_2([1077, 1081 | Rest]) -> Rest; % ей -> ""
step1_2([1091, 1081 | Rest]) -> Rest; % уй -> ""
step1_2([1080, 1083 | Rest]) -> Rest; % ил -> ""
step1_2([1099, 1083 | Rest]) -> Rest; % ыл -> ""
step1_2([1080, 1084 | Rest]) -> Rest; % им -> ""
step1_2([1099, 1084 | Rest]) -> Rest; % ым -> ""
step1_2([1077, 1085 | Rest]) -> Rest; % ен -> ""
step1_2([1103, 1090 | Rest]) -> Rest; % ят -> ""
step1_2([1080, 1090 | Rest]) -> Rest; % ит -> ""
step1_2([1099, 1090 | Rest]) -> Rest; % ыт -> ""
step1_2([1091, 1102 | Rest]) -> Rest; % ую -> ""
step1_2([1102 | Rest]) -> Rest; % ю -> ""

% NOUN 1. it's becauise of pattern matching nature
step1_2([1080, 1103, 1084, 1080 | Rest]) -> Rest; % иями -> ""
step1_2([1103, 1084, 1080 | Rest]) -> Rest; % ями -> ""
step1_2([1072, 1084, 1080 | Rest]) -> Rest; % ами -> ""
step1_2([1080, 1103, 1084 | Rest]) -> Rest; % иям -> ""
step1_2([1080, 1077, 1081 | Rest]) -> Rest; % ией -> ""
step1_2([1080, 1077, 1084 | Rest]) -> Rest; % ием -> ""
step1_2([1080, 1103, 1093 | Rest]) -> Rest; % иях -> ""
...
step1_2([1080, 1077 | Rest]) -> step1_3(Rest); % ие -> ""
step1_2([1099, 1077 | Rest]) -> step1_3(Rest); % ые -> ""
step1_2([1086, 1077 | Rest]) -> step1_3(Rest); % ое -> ""
%step1_2([1077, 1081 | Rest]) -> step1_3(Rest); % ей -> "" 118
step1_2([1080, 1081 | Rest]) -> step1_3(Rest); % ий -> ""
step1_2([1099, 1081 | Rest]) -> step1_3(Rest); % ый -> ""
step1_2([1086, 1081 | Rest]) -> step1_3(Rest); % ой -> ""
step1_2([1077, 1084 | Rest]) -> step1_3(Rest); % ем -> ""
%step1_2([1080, 1084 | Rest]) -> step1_3(Rest); % им -> "" 122
%step1_2([1099, 1084 | Rest]) -> step1_3(Rest); % ым -> ""
step1_2([1086, 1084 | Rest]) -> step1_3(Rest); % ом -> ""
step1_2([1080, 1093 | Rest]) -> step1_3(Rest); % их -> ""
step1_2([1099, 1093 | Rest]) -> step1_3(Rest); % ых -> ""
%step1_2([1091, 1102 | Rest]) -> step1_3(Rest); % ую -> ""
step1_2([1072, 1103 | Rest]) -> step1_3(Rest); % ая -> ""
step1_2([1103, 1103 | Rest]) -> step1_3(Rest); % яя -> ""
step1_2([1086, 1102 | Rest]) -> step1_3(Rest); % ою -> ""
step1_2([1077, 1102 | Rest]) -> step1_3(Rest); % ею -> ""

% NOUN 2. а ев ов ие ье е иями ями ами еи ии и ией ей ой ий й иям ям ием ем ам ом о у ах иях ях ы ь ию ью ю ия ья я -> ""
step1_2([1077, 1074 | Rest]) -> Rest; % ев -> ""
step1_2([1086, 1074 | Rest]) -> Rest; % ов -> ""
%step1_2([1080, 1077 | Rest]) -> Rest; % ие -> "" 143
step1_2([1100, 1077 | Rest]) -> Rest; % ье -> ""
step1_2([1077, 1080 | Rest]) -> Rest; % еи -> ""
step1_2([1080, 1080 | Rest]) -> Rest; % ии -> ""
%step1_2([1077, 1081 | Rest]) -> Rest; % ей -> "" 118
%step1_2([1086, 1081 | Rest]) -> Rest; % ой -> "" 149
%step1_2([1080, 1081 | Rest]) -> Rest; % ий -> "" 147
step1_2([1103, 1084 | Rest]) -> Rest; % ям -> ""
%step1_2([1077, 1084 | Rest]) -> Rest; % ем -> "" 150
step1_2([1072, 1084 | Rest]) -> Rest; % ам -> ""
%step1_2([1086, 1084 | Rest]) -> Rest; % ом -> ""
step1_2([1072, 1093 | Rest]) -> Rest; % ах -> ""
step1_2([1103, 1093 | Rest]) -> Rest; % ях -> ""
step1_2([1080, 1102 | Rest]) -> Rest; % ию -> ""
step1_2([1100, 1102 | Rest]) -> Rest; % ью -> ""
step1_2([1080, 1103 | Rest]) -> Rest; % ия -> ""
step1_2([1100, 1103 | Rest]) -> Rest; % ья -> ""
...
step1_3([1077, 1084, V | Rest]) when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; %(а/я) ем => (а/я)
step1_3([1085, 1085, V | Rest]) when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; %(а/я) нн => (а/я)
step1_3([1074, 1096, V | Rest]) when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; %(а/я) вш => (а/я)
step1_3([1102, 1097, V | Rest]) when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; %(а/я) ющ => (а/я)
step1_3([1097, V | Rest])       when (V =:= 1103) orelse (V =:= 1072) -> [V | Rest]; %(а/я) щ => (а/я)
%% PARTICIPLE group2 ивш ывш ующ -> ""
step1_3([1080, 1074, 1096 | Rest]) -> Rest; % "ивш" -> ""
step1_3([1099, 1074, 1096 | Rest]) -> Rest; % "ывш" -> ""
step1_3([1091, 1102, 1097 | Rest]) -> Rest; % "ующ" -> ""
%% ALL OTHER
step1_3(W) -> W.
